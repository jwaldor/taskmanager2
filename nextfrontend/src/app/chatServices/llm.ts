...
export async function suggestTasks(messages: MessagesType): Promise<{
  tasks: { title: string; description: string; state: TaskState }[];
}> {
  const maxRetries = 5; // Maximum number of retries
  let attempt = 0;
        ...
        messages: [
          {
            role: "system",
            content:
              "The user has put you in task suggestion mode. Given what has happened recently in the conversation, suggest tasks that the user would find useful.",
          },
          ...messages,
        ], // Cast to the correct type
        model: "gpt-4o-mini",
